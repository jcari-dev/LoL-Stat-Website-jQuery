...
//     let $api_key = "?api_key=RGAPI-71a14967-c77a-40f7-a897-fdc81192c682"; // my api key
...
$(()=>{let t,a,e,s,n,i,o,l,p,c,r,m,d,g,h,u,f,b,D,k,L,v,S,I,C,y,T,w,R,j,O,x,A,G,W,_,K,N,E,M,U,P,B,F,J,H,V,q,Y,z,Q,X,Z,tt,at,et,st,nt,it,ot,lt,pt,ct,rt,mt,dt,gt,$t,ht,ut,ft,bt=0,Dt=[],kt=[],Lt=[],vt=0,St=0,It=0,Ct=0,yt="",Tt=0,wt="",Rt=0,jt=0,Ot=0,xt="https://",At=".api.riotgames.com/lol/summoner/v4/summoners/by-name/",Gt="?api_key=RGAPI-71a14967-c77a-40f7-a897-fdc81192c682",Wt="https://cross-anywhere.herokuapp.com/",_t=".api.riotgames.com/lol/league/v4/entries/by-summoner/",Kt=".api.riotgames.com/lol/match/v4/matchlists/by-account/",Nt="https://ddragon.leagueoflegends.com/cdn/11.7.1/img/spell/",Et=()=>{$.ajax({type:"GET",url:Wt+xt+x+Kt+A+Gt}).then(function(t){console.log(t),console.log(xt+x+Kt+A+Gt),B=t.totalGames;for(let a=0;a<10;a++)Dt.push(t.matches[a].gameId);for(let a=0;a<10;a++)kt.push(t.matches[a].gameId);pt=`https://ddragon.leagueoflegends.com/cdn/11.7.1/img/profileicon/${lt}.png`,Mt()})},Mt=()=>{$.ajax({type:"GET",url:Wt+xt+x+_t+G+Gt}).then(function(t){console.log(t),console.log(xt+x+_t+G+Gt);for(let a of t)$userRankLoses=t[0].losses,$userRankWins=t[0].wins,$totalRankMatches=$userRankLoses+$userRankWins,$tRankWrResult=($userRankWins/$totalRankMatches*100).toFixed(2),$userrankedwinrate=`Ranked Winrate: ${$tRankWrResult}% <br>`;""==t?(P="Unranked",Pt()):(W=t[0].tier,_=t[0].rank,K=`${W}/`,N=`${_}/`,E=`Rank:  ${W} ${t[0].rank}`,Ut())})},Ut=()=>{$.ajax({type:"GET",url:Wt+xt+x+".api.riotgames.com/lol/league-exp/v4/entries/RANKED_SOLO_5x5/"+K+N+Gt}).then(function(t){console.log(t),console.log(xt+x+".api.riotgames.com/lol/league-exp/v4/entries/RANKED_SOLO_5x5/"+K+N+Gt);for(let a=0;a<t.length;a++)vt+=t[a].wins,St+=t[a].losses,$tierWinrate=(vt/(It=vt+St)*100).toFixed(2);Pt()})},Pt=()=>{M();for(let R=0;R<Dt.length;R++)$.ajax({type:"GET",url:Wt+xt+x+".api.riotgames.com/lol/match/v4/matches/"+Dt[R]+Gt}).then(function(j){console.log(j),console.log(xt+x+".api.riotgames.com/lol/match/v4/matches/"+Dt[R]+Gt),H=[R];for(let t=0;t<10;t++)if(j.participantIdentities[t].player.currentAccountId==A){J=[t],F=j.participants[J].championId;break}for(let t=0;t<10;t++){1==j.participants[J].stats.win?(Lt.push("W"),Ct+=1):Lt.push("L"),Tt=Lt.length;break}a=1==j.participants[J].stats.win?"Victory!":"Defeat!",t=1==j.participants[J].stats.firstBloodKill?"You got first blood!":"",ct=Object.entries($daListSum.data);for(let t=0;t<14;t++)j.participants[J].spell1Id==ct[t][1].key&&(rt=ct[t][0],dt=`${Nt}${rt}.png`),j.participants[J].spell2Id==ct[t][1].key&&(mt=ct[t][0],gt=`${Nt}${mt}.png`);yt=(100*Ct/Tt).toFixed(2),e=j.participants[J].spell1Id,s=j.participants[J].spell2Id,n=j.participants[J].stats.champLevel,i=j.participants[J].stats.magicDamageDealt.toLocaleString(),o=j.participants[J].stats.magicDamageDealtToChampions.toLocaleString(),l=j.participants[J].stats.physicalDamageDealt.toLocaleString(),p=j.participants[J].stats.physicalDamageDealtToChampions.toLocaleString(),c=j.participants[J].stats.trueDamageDealt.toLocaleString(),r=j.participants[J].stats.trueDamageDealtToChampions.toLocaleString(),m=j.participants[J].stats.longestTimeSpentLiving,d=j.participants[J].stats.pentaKills,g=j.participants[J].stats.quadraKills,h=j.participants[J].stats.perk0,u=j.participants[J].stats.timeCCingOthers,f=j.participants[J].stats.totalDamageDealt.toLocaleString(),$totalDmgDealtToChampion=j.participants[J].stats.totalDamageDealtToChampions.toLocaleString(),b=j.participants[J].stats.totalDamageTaken.toLocaleString(),D=j.participants[J].stats.totalHeal.toLocaleString(),k=j.participants[J].stats.totalTimeCrowdControlDealt,L=j.participants[J].stats.tripleKills,v=j.participants[J].stats.turretKills,S=j.participants[J].stats.wardsPlaced,I=j.participants[J].stats.doubleKills,C=j.participants[J].stats.goldSpent.toLocaleString(),y=j.participants[J].stats.firstBloodKill,T=j.participants[J].stats.firstBloodAssist,w=j.participants[J].stats.damageSelfMitigated,$("#rank").text(`Winrate of the last ${Tt} games, ${yt}%`),wt=Object.values(U.data),$assists=j.participants[J].stats.assists,$kills=j.participants[J].stats.kills,$deaths=j.participants[J].stats.deaths,q=j.gameCreation,Y=new Date(q).toLocaleDateString("en-us"),z=Math.round(j.gameDuration/60),Q=Math.round(j.gameDuration%60),X=j.participants[J].stats.goldEarned,Z=X.toLocaleString(),$items=Object.values(Object.values(Object.entries(j.participants[J].stats)));for(let t=0;t<wt.length;t++)if(j.participants[J].championId==wt[t].key){V=t,$championName=wt[t].id;break}$matchsInfoAppend();for(let t=2;t<9;t++){if(at=$items,et=t,Rt=1,$items[t][1]>=7e3&&$items[t][1]<7023)tt=!0;else if(0!==$items[t][1]){nt=!0;for(let a=0;a<$daListItem.length&&(nt=!0,jt=0,it=$daListItem[st=a][Rt].name,$t=$daListItem[a][1].description,ht=$daListItem[a][1].name,ut=$daListItem[a][1].gold.total.toLocaleString(),ft=$daListItem[a][1].gold.sell.toLocaleString(),$daListItem[a][0]!=$items[t][1]);a++);}$appendItems()}})};M=(()=>{$("#accountinfo").append(`<img class="usericon" src=${pt}><br>`),$("#accountinfo").append(`IGN: ${j} <br> Summoner Level: ${O}<br>`),$("#accountinfo").append("Total Games on S13: "+B),"Unranked"==P?$("#accountinfo").append("<br>Unranked<br>"):($("#accountinfo").append(`<br>${E}`),$("#accountinfo").append(`<br>Average ${W} ${_} Winrate: ${$tierWinrate}%<br>`)),$("#accountinfo").show()}),$matchsInfoAppend=(()=>{$("#matchlist").append(`<div class="matchnum" id=match${H}>\n        <br>\n        <a href="https://leagueoflegends.fandom.com/wiki/${wt[V].id}/LoL" target="_blank"> \n        <img class="champImg" src="img/champion//${wt[V].id}.png"/></a>\n        <br>\n        <img class="summIcons" src="${dt}"><img class="summIcons" src="${gt}"<br>\n        <p>${a}</p>\n        <p>Champion: ${$championName}</p>\n        <p>Level: ${n}</p>\n        <p>Date: ${Y}</p>\n        <p>Game Duration: ${z} mins, ${Q} sec.</p>\n        <p>Gold Earned: ${Z} </p>\n        <p> Kills: ${$kills} | Deaths: ${$deaths} | Assists: ${$assists}</p>\n        <button id="buttonshow">Match Details</button><br>\n        <div class="details hidden">\n        <p>Gold Spent: ${C}\n        <p>Damage Done: ${f}</p>\n        <p Damage Done to Champions: ${$totalDmgDealtToChampion}</p>\n        <p>Magic Damage Done: ${i}</p>\n        <p>Magic Damage Done to Champions: ${o}</p>\n        <p>Physical Damage Done: ${l}</p>\n        <p>Physical Damage Done to Champions: ${p}</p>\n        <p>Longest Time Spent Alive: ${m} secs.</p>\n        <p>Time Applying CC: ${u} secs.</p>\n        <p>Time Being CCed: ${k} secs.</p>\n        <p>Healing Done: ${D}</p>\n        <p>Wards Placed: ${S}</p>\n        </div>\n        <br>\n        </div>\n        `),"Victory!"==a?($(".matchnum").addClass("matchnumblue"),$(".matchnum").removeClass("matchnum")):($(".matchnum").addClass("matchnumred"),$(".matchnum").removeClass("matchnum"))}),$appendItems=(()=>{0==at[et][Rt]||(1==tt?$(`#match${H}`).append(`<img class="itemImg tooltip" src="https://ddragon.bangingheads.net/cdn/latest/img/item/${$items[et][Rt]}.png">`):1==nt&&$(`#match${H}`).append(`<div class="tooltip"><a href="https://leagueoflegends.fandom.com/wiki/${it}" target="_blank">\n            <img class="itemImg"  src="img/item/${at[et][Rt]}.png">\n            <span class="tooltiptext">Name: ${ht}<br><br>Cost: ${ut}g<br><br>Sells For: ${ft}g<br><br>${$t}</span></div>`))}),$(document).on("click","#buttonshow",function(){$(".details").hasClass("hidden")?($(".details").removeAttr("display","none"),$(".details").show(),$(".details").removeClass("hidden")):($(".details").hide(),$(".details").addClass("hidden"))}),$("#submitbtn").on("click",t=>{t.preventDefault(),R=$("#sname").val(),$snameClear=$("#sname").val(),x=$("#server").val(),R.length<3?alert("Too short, please, try again!"):(R=R.replace(/ /g,"%20"),$call=$.ajax({type:"GET",url:Wt+xt+x+At+R+Gt}).then(function(t){console.log(t),console.log(xt+x+At+R+Gt),j=t.name,O=t.summonerLevel,A=t.accountId,G=t.id,lt=t.profileIconId,$.getJSON("https://ddragon.leagueoflegends.com/cdn/11.7.1/data/en_US/champion.json",function(t){U=t}),$.getJSON("https://ddragon.leagueoflegends.com/cdn/11.7.1/data/en_US/item.json",function(t){$daListItem=Object.entries(t.data)}),$.getJSON("https://ddragon.leagueoflegends.com/cdn/11.7.1/data/en_US/profileicon.json",function(t){ot=t}),$.getJSON("https://ddragon.leagueoflegends.com/cdn/11.7.1/data/en_US/summoner.json",function(t){$daListSum=t}),Et()})),1==(Ot=1)&&(console.log("I love my class uwu <3"),$("#searchAgain").css("visibility","visible")),bt=1,1==kata&&(console.log("got here"),$("#kata").css("visibility","visible"))}),$("#searchAgain").on("click",function(){location.reload()})});